...
filename = 'TestPosD11';
NumBlocks_sim = [8 8];    % Number of scene patches [hor, vert]
D = 1.03; % 相机和墙面之间的距离

% OCCLUDER DEFINITION flat障碍物的长，宽，高 宽当然就是0了，因为没什么厚度
Occ_size = [0.077 0 0.077]; % Size of the flat occluder (x,y,z)
...
% occluder plane.
% 最终得到的y-position是D-p_y, p_y是障碍物到墙面的距离
if useEstimatedOccPos
    ...
    Occ_LLcorner = [1 D-0.23 0.912];  % True/measured occluder position 实际测量的障碍物的位置
end
...
Mon_Offset = [0.0820 0.756];        % (x,z)-position of the lower-right edge of usuable portion of LCD screen 因为显示的图案不一定能占满整个显示屏
...
ScreenSize = [0.345 0.195];        % Size of the LCD display [m] 完整的显示屏显示大小
ScreenResolution = [3840 2160];     % [hor, ver] pixels
NumPixelsPerMonitorBlock = 270;      % Number of monitor pixels in each hidden-scene patch
PixelSize_m = (ScreenSize./ScreenResolution);   % Dimension of each scene patch [m] 每个像素对应为多少米

% 因为我是直接在左上角显示的，所以不需要补齐这个
